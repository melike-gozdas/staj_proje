...
@section('title', 'Duyuru '.(isset($duyuru) ? 'Düzenleme': 'Ekleme'))
@section('css')
...
@endsection
@section('PageAdı')
Duyuru {{isset($duyuru) ? 'Düzenleme' : 'Ekleme'}}
@endsection
@section('content')
<form action="{{route('duyuru_kaydet')}}" method="post" enctype="multipart/form-data">
	@csrf
	<div class="card card-outline card-info">
		<div class="card-header">
			<div class="form-group">
				...
				@else
				<input type="text" hidden name="id" value="{{$duyuru->id}}">
				@endif
				<!--
				<input type="text" hidden name="id" value="yeni">
				-->
				<div class="row">
					<input type="text" name="duyuru_baslik" class="form-control col-md-10" placeholder="Duyuru başlığını yazınız..." style="left:15px;" value="{{$duyuru->baslik ?? ''}}">
					<div class="form-check"  style="left:50px;">
						@if(isset($duyuru->durum))
						<input name="duyuru_durum" class="form-check-input" type="checkbox" value="{{$duyuru->durum}}"
						@if($duyuru->durum) checked @endif >
		...
		<div class="card-body">
			<div class="col-lg-4">
				<input type="file" class="form-control" name="image" value="{{$duyuru->resim ?? ''}}">
				<br>
				@if(isset($duyuru->resim))
				<img src="{{$duyuru->resim}}" width="300">
				@endif
			</div>
			<br>
			<div class="col-lg-10">
				<textarea id="summernote" name="duyuru_metni" value="{{$duyuru->icerik ?? ''}}">
				@if(isset($duyuru->icerik))
				{{$duyuru->icerik}}
				@else
